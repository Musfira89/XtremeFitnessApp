...
const UserProfile = () => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        const formattedUser = {
          ...
          hasCompletedQuestionnaire: user.hasCompletedQuestionnaire ? "Yes" : "No",
        };
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1100, margin: "0 auto", backgroundColor: "#fff" }}>
      ...
      <Paper elevation={0} sx={{ p: 3, borderRadius: 2, border: "1px solid lightgrey" }}>
        <Typography variant="h6" gutterBottom>Profile Details</Typography>
        <Typography color="textSecondary" sx={{ mb: 3 }}>User information</Typography>
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <TextField label="Full Name" value={userData.fullName} fullWidth disabled />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField label="Email" value={userData.email} fullWidth disabled />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField label="Contact" value={userData.contact} fullWidth disabled />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField label="Location" value={userData.location} fullWidth disabled />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField label="Plan Name" value={userData.plan} fullWidth disabled />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField label="Subscription Status" value={userData.subscriptionStatus} fullWidth disabled />
          </Grid>
          <Grid item xs={12} sm={6}>
            <TextField
