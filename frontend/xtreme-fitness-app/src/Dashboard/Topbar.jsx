...
const Topbar = ({ toggleSidebar }) => {
  ...
  useEffect(() => {
    const fetchPlanDetails = async () => {
      try {
        ...
        setExpiryDate(
          response.data.planExpiry ? new Date(response.data.planExpiry) : null
        );
        setTrialExpiryDate(
          response.data.trialExpiryDate
            ? new Date(response.data.trialExpiryDate)
            : null
        );
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const trialTimerCleanup = updateCountdown(
      trialExpiryDate,
      setTrialTimeLeft
    );
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          ...
          <Link
            ...
            className="mt-1 text-sm underline text-gray-200 hover:text-gray-100"
          >
            Join Meeting
          </Link>
        </div>
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          {trialTimeLeft && (
            ...
            >
              {trialTimeLeft === "Expired"
                ? "Trial Expired!"
                : `Trial expires in: ${trialTimeLeft}`}
            </div>
          )}
          <Link
            to={`/planpage/${userId}`}
