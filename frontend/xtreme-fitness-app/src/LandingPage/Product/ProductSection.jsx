...
const ProductSection = () => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-4xl font-bold text-gray-800">Our Fitness Picks</h2>
        <p className="text-gray-600 mt-3 text-lg">
          High-quality supplements and equipment for your fitness goals.
        </p>
        <div className="w-20 h-[3px] bg-red-600 mx-auto mt-2"></div>
      </motion.div>

      {/* Product Grid */}
      <motion.div
        className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0 mt-12 px-6 lg:px-0"
        initial={{ opacity: 0 }} // Start with no opacity
        whileInView={{ opacity: 1 }} // Fade in when in view
        transition={{ duration: 1 }} // Animation duration
        viewport={{ once: true }} // Trigger animation once when entering the viewport
      >
        {products.slice(0, 3).map((product, index) => (
          <motion.div
            ...
            style={{ width: "340px", height: "460px" }} // Increased size
          ...
          >
            ...
            {/* Product Image */}
            <div className="relative h-72 w-full overflow-hidden rounded-md flex justify-center items-center">
              <img
                src={product.image}
                alt={product.name}
            ...
            <div className="mt-4 flex flex-col items-center text-center">
              ...
              <button
                className="mt-4 px-6 py-2 bg-red-600 text-white rounded-md font-medium shadow-sm hover:bg-red-700 hover:shadow-md transition"
      ...
      <div className="mt-12 text-center">
        <button
          ...
          className="px-10 py-3 border-2 border-red-600 text-red-600 text-lg font-semibold rounded-md hover:bg-red-600 hover:text-white transition"
        >
          See More Products
        </button>
      </div>
