import proteinImage from "../../assets/products/protein.png";
import testBoosterImage from "../../assets/products/test-booster.jpg";
import proteinBarsImage from "../../assets/products/protein-bars.png";
import fatBurnerImage from "../../assets/products/fat-burner.jpg";
import dumbbellsImage from "../../assets/products/dumbbells.jpg";
import workoutMatImage from "../../assets/products/workout-mat.jpg";
import workoutBenchImage from "../../assets/products/workout-bench.jpg";
import exerciseBikeImage from "../../assets/products/exercise-bike.jpg";
import weightedjumprope from "../../assets/products/weightedjumprope.jpg"
import barbell from "../../assets/products/barbell.jpg"
import nighttimeFatBurner from "../../assets/products/nighttimeFatBurner.jpg"
import treadmill from "../../assets/products/treadmill.jpg"
import fatmachine from "../../assets/products/fatmachine.jpg"

const products = [
    {
      name: "Protein",
      image: proteinImage,
      price: "$39.99",
      affiliateLink: "https://amzn.to/3Vob2l5", // Replace with your actual link
    },
    {
      name: "Test Booster",
      image: testBoosterImage,
      price: "$29.99",
      affiliateLink: "https://amzn.to/3VlCJLj", // Replace with your actual link
    },
    {
      name: "Protein Bars",
      image: proteinBarsImage,
      price: "$19.99",
      affiliateLink: "https://amzn.to/4f5DKOB", // Replace with your actual link
    },
    {
      name: "Fat Burner",
      image: fatBurnerImage,
      price: "$24.99",
      affiliateLink: "https://amzn.to/41gNbrj", // Replace with your actual link
    },
    {
      name: "Dumbbells",
      image: dumbbellsImage,
      price: "$59.99",
      affiliateLink: "https://amzn.to/3OCRzJJ", // Replace with your actual link
    },
    {
      name: "Workout Mat",
      image: workoutMatImage,
      price: "$19.99",
      affiliateLink: "https://amzn.to/4fVNXOS", // Replace with your actual link
    },
    {
      name: "Workout Bench",
      image: workoutBenchImage,
      price: "$149.99",
      affiliateLink: "https://amzn.to/3Zww5o5", // Replace with your actual link
    },
    {
      name: "Exercise Bike",
      image: exerciseBikeImage,
      price: "$399.99",
      affiliateLink: "https://amzn.to/3OE0pXA", // Replace with your actual link
    },
    {
      name: "Weighted Jump Rope",
      image: weightedjumprope,
      price: "$59.99",
      affiliateLink: "https://amzn.to/40yyvly", // Replace with your actual link
    },
    {
      name: "Barbell",
      image: barbell,
      price: "$19.99",
      affiliateLink: "https://amzn.to/40xvj9x", // Replace with your actual link
    },
    {
      name: "Night-time Fat Burner",
      image: nighttimeFatBurner,
      price: "$149.99",
      affiliateLink: "https://amzn.to/40Nc4dx", // Replace with your actual link
    },
    {
      name: "Treadmill",
      image: treadmill,
      price: "$399.99",
      affiliateLink: "https://amzn.to/4gckj7w", // Replace with your actual link
    },
    {
      name: "Body fat weight scale",
      image: fatmachine,
      price: "$399.99",
      affiliateLink: "https://amzn.to/3ECLoDL", // Replace with your actual link
    },
  ];
  
  export default products;
  